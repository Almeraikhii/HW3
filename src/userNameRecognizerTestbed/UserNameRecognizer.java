...
public class UserNameRecognizer {
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						...
						// Removed the check for 0-9 since only alphabetic characters are allowed as the first character
					nextState = 1;
			...
			case 1: 
				...
				// . -> State 2
				// Edited to include - and _ as options to go to State 2
				else if ((currentChar == '.') || 
						(currentChar == '-') || 
						(currentChar == '_')) {	// Check for . - or _
					nextState = 2;
			...
			case 2: 
				...
				// If it is none of those characters, the FSM halts
				// No change is needed here, since . - and _ are all not alphanumeric characters
				else 
					running = false;
		...
		case 0:
			...
			//Edited to specify only A-Z or a-z, removing 0-9 from the String.
			userNameRecognizerErrorMessage += "A UserName must start with A-Z or a-z.\n";
			System.out.println(userNameRecognizerErrorMessage);
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				// Edited to specify all valid characters at this state
				userNameRecognizerErrorMessage += 
					"A UserName may only contain the characters A-Z, a-z, 0-9, '.', '-', or '_'.\n";
				System.out.println(userNameRecognizerErrorMessage);
				return userNameRecognizerErrorMessage;
			}
			else {
					...
					userNameRecognizerErrorMessage = "UserName is valid\n";
					System.out.println(userNameRecognizerErrorMessage);
		...
		case 2:
			...
			// Edited to mention minus signs and underscores
			userNameRecognizerErrorMessage +=
				"A UserName character after a period, minus sign, or underscore must be A-Z, a-z, or 0-9.\n";
			System.out.println(userNameRecognizerErrorMessage);
			return userNameRecognizerErrorMessage;
